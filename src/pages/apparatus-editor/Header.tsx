...
const Header = ({
  ...
  return (
    <header
      className={cn(
        'bg-background dark:bg-background border-b border-gray-200 dark:border-gray-600',
        className
      )}
    >
      ...
      <TooltipProviderMemo>
        <div
          className={cn(
            'bg-background dark:bg-background flex items-center gap-1 px-3 py-2',
            className
          )}
        >
          {/* File Operations */}
          ...
          {/* Text Alignment */}
          <div className="flex items-center gap-1">
            <TextAlignment textAlign={textAlign} setTextAlign={setTextAlign} />
            <SeparatorMemo />
          </div>
          ...
          {/* View Options */}
          <div className="flex items-center gap-1">
            <ViewOptions isPreview={false} setPreview={handleSetPreview} />
            <SeparatorMemo />
          </div>
...
const HeadingLevelSelect = memo(
  ({
    ...
    return (
      <TooltipMemo>
        <Select
          value={headingLevel === 0 ? 'body' : `h${headingLevel}`}
          onValueChange={(value) => {
            if (value === 'body') {
              setHeadingLevel(0)
            } else {
...
const FontSizeSelect = memo(
  ({ fontSize, setFontSize }: { fontSize: number; setFontSize: (fontSize: number) => void }) => {
    return (
      <TooltipMemo>
...
const InsertOptions = memo(
  ({
    ...
    <>
      <TooltipMemo>
        <TooltipTriggerMemo asChild>
          <Button variant={isLink ? 'default' : 'ghost'} size="sm" onClick={() => setLink(!isLink)}>
...
const ViewOptions = memo(
  ({ isPreview, setPreview }: { isPreview: boolean; setPreview: (isPreview: boolean) => void }) => {
    return (
      <>
        <TooltipMemo>
          <TooltipTriggerMemo asChild>
...
const BreadcrumbMemo = memo(() => {
  return (
    <Breadcrumb>
      <BreadcrumbList>
        <BreadcrumbItem>
          <BreadcrumbPage className="line-clamp-1 text-xl font-black">Apparatus Editor</BreadcrumbPage>
        </BreadcrumbItem>
      </BreadcrumbList>
    </Breadcrumb>
  )
